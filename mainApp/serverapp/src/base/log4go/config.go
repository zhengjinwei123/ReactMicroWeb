...
func (log Logger) LoadConfiguration(filename string, contents []byte) {
	log.Close()
...
//为日志文件增加进程id，防止同时起多个进程时日志文件共用问题
func (log Logger) LoadConfigurationWithPid(filename string, contents []byte) {
	log.Close()
	...
	for _, xmlfilt := range xc.Filter {
		...
		for nI := range xmlfilt.Property {
			if xmlfilt.Property[nI].Name == "filename" {
				xmlfilt.Property[nI].Value += fmt.Sprintf("%d", os.Getpid())
				break
			}
		}
...
func xmlToFileLogWriter(filename string, props []xmlProperty, enabled bool) (*FileLogWriter, bool) {
	...
	flw := NewFileLogWriter(file, rotate).SetHeadFoot("<log created=\"%D %T\">", "</log>")
